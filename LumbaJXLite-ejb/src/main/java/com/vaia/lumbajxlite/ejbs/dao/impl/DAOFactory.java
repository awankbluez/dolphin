...
import com.vaia.lumbajxlite.ejbs.util.dao.DAOConfigurationException;
import com.vaia.lumbajxlite.ejbs.util.dao.DAOProperties;
 ...
 * DAO getters. <p> This class requires a properties file named 'dao.properties'
 * in the classpath with among others the following properties:
 * <pre>
 * name.url *
 * name.driver
 * name.username
 * name.password
 * </pre> Those marked with * are required, others are optional and can be left
 * away or empty. Only the username is required when any password is specified.
 * <ul> <li>The 'name' must represent the database name in
 * {@link #getInstance(String)}.</li> <li>The 'name.url' must represent either
 * the JDBC URL or JNDI name of the database.</li> <li>The 'name.driver' must
 * represent the full qualified class name of the JDBC driver.</li> <li>The
 * 'name.username' must represent the username of the database login.</li>
 * <li>The 'name.password' must represent the password of the database
 * login.</li> </ul> If you specify the driver property, then the url property
 * will be assumed as JDBC URL. If you omit the driver property, then the url
 * property will be assumed as JNDI name. When using JNDI with username/password
 * preconfigured, you can omit the username and password properties as well. <p>
 ...
 * </pre> Here is a basic use example:
 * <pre>
...
public abstract class DAOFactory {
    ...
    private static final String PROPERTY_JNDI = "jndi";
     ...
     * @throws DAOConfigurationException If the database name is null, or if the
     * properties file is missing in the classpath or cannot be loaded, or if a
     * required property is missing in the properties file, or if either the
    ...
    public static DAOFactory getInstance(String name) throws DAOConfigurationException {
        ...
        String jndi = properties.getProperty(PROPERTY_JNDI, true);
        DAOFactory instance;
        ...
        if (driverClassName != null) {
            ...
            try {
                ...
                throw new DAOConfigurationException(
                        "DataSource '" + jndi + "' is missing in JNDI.", e);
            }
            if (username != null) {
    ...
    public <DAO extends BaseDAO> DAO getDAOImpl(Class<DAO> daoInterface) throws DAOConfigurationException {
        ...
        if (!daoInterface.isInterface()) {
            throw new DAOConfigurationException("Class '" + daoInterfaceName + "'" + " is actually not an Interface.");
        }

        String daoClassName = daoInterfaceName.concat("Impl");
